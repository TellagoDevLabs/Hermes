...
namespace TellagoStudios.Hermes.Business.Subscriptions
{
    public class CreateSubscriptionCommand : ICreateSubscriptionCommand
    {
        ...
        {
            if (!queryValidator.IsValid(subscription.Filter)) throw new ValidationException(string.Format(Messages.InvalidFilter, subscription.Filter));
            if(subscription.TargetId == null) throw new ValidationException(Messages.TargetIdMustNotBeNull);
